...
1. **Environment Variables**:
   ...
   cp .env.example .env
   ```
   Open `.env` in your text editor and provide your Twitter credentials, Telegram tokens, and LLM API key.

2. **Bot Settings**:
   Open `config.yaml` to configure the bot's behavior.
   - Set your `niche` (e.g., "Artificial Intelligence").
   - Set `tone_of_voice` for the generated tweets.
   - Set `mode` to `auto` to let the bot find its own sources, or `manual` to strictly use the sources defined in the file.
   - Configure your LLM endpoint in the `llm` section (default is set to NVIDIA's DeepSeek V4 Pro, but you can change `base_url` and `model_name` to use GPT-4, Claude, or any local model).
